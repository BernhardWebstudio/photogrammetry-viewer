...
export class ViewerElement3D extends ModelViewerElement {
    ...
    constructor() {
      ...
      singleClickEventHandler.on('pointerevent-is-hold-event', () => {
        console.log('Single click over'), this[$userInputElement].style.cursor = 'grabbing';
      });
    }
    ...
    private _handleModelLoaded(): void {
      ...
      const negXAxisLabel = new Sprite(getSpriteMaterial(new Color(1, 0.2, 0.2), '-x'));
      ...
      const negYAxisLabel = new Sprite(getSpriteMaterial(new Color(0.2, 1.0, 0.2), '-y'));
      ...
      const negZAxisLabel = new Sprite(getSpriteMaterial(new Color(0.2, 0.2, 1.0), '-z'));
